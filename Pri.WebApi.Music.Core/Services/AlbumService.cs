...
namespace Pri.WebApi.Music.Core.Services
{
    public class AlbumService : IAlbumService
    {
        ...
        public AlbumService(IAlbumRepository albumRepository,IImageService imageService)
        {
        ...
        public async Task<bool> AddAsync(string name, DateTime releaseDate, IFormFile image, int artistId)
        {
            var newAlbum = new Album 
            {
                ...
                Image = await _imageService.AddOrUpdateImageAsync<Album>(image),
                ArtistId = artistId
            };
            try 
            {
        ...
        {
            var itemResultModel = new ItemResultModel<Album>
            {
                Items = await _albumRepository.ListAllAsync(),
                Error = "",
                IsSuccess = true
            };
            foreach(var item in itemResultModel.Items)
            {
                item.Image = _imageService.GetImageUrl<Album>(item.Image);
            }
        ...
        public async Task<ItemResultModel<Album>> GetByIdAsync(int id)
        {
            ...
            //add image url
            album.Image = _imageService.GetImageUrl<Album>(album.Image);
        ...
        public async Task<bool> UpdateAsync(int id, string name, DateTime releaseDate, IFormFile image, int artistId)
        {
            ...
            albumToUpdate.Image = await _imageService.AddOrUpdateImageAsync<Album>(image,albumToUpdate.Image);
            try 
            {
